...
此程序解决了问题二，加入了在特定时段对燃油车进入绿色区域约束。
主要思路：
...
最终生成的实验报告在当前目录下：
    - Q2_单车单趟明细排班方案.txt:  包含每个任务点的分配车辆、发车时间、路径和成本等详细信息。
    - Q2一车多趟排班方案.txt:      包含基于单车单趟方案合并后的多趟排班方案;
                                显示每辆实体车的趟次、发车时间、路径和成本等信息，并统计节省的启动费和最终总成本。
日志文件：
    - logs/q2.log: 包含算法执行过程中的详细日志
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/q2.log", encoding="utf-8"),
...
class TDVRP_Ultimate_Solver:
    ...
    def load_data(self, cust_file, dist_file):
        ...
        vid_counter = 1000  # 虚拟节点起始编号
        orig_to_vids = {0: [0]}  # 真实ID到虚拟ID组的映射表

        # 阈值 1200kg/6.0m3，确保适配车队中最小的 1250kg/6.5m3 车型
        ...
        for _, row in df_cust.iterrows():
            ...
            while temp_w > SAFE_W or temp_v > SAFE_V:
                ...
                chunk_w = temp_w * ratio
                chunk_v = temp_v * ratio
            ...
            # 处理剩余的尾数节点
            if temp_w > 0.001 or temp_v > 0.001:
        ...
        logging.info(
            f"数据加载完成。原始客户: {len(df_cust)}，拆分后任务点: {len(self.customers) - 1}"
        )
            ...
            for i in range(len(path) - 1):
                ...
                if nxt != 0:
                    ...
                    if veh["type"] == 0 and self.customers[nxt]["is_green"] == 1:
                        serv_start = max(arr_t, self.customers[nxt]["ready"])
                        serv_end = serv_start + self.service_time
                        # 检查 08:00(480) 到 16:00(960) 期间是否处于绿区
                        if max(arr_t, 480) < min(serv_end, 960):
                            sub += 1000000.0
...
def generate_mtvrp_schedule(input_file, coords, output_file="一车多趟排班方案.txt"):
    ...
    with open(output_file, "w", encoding="utf-8") as f_out:
        f_out.write(f"{'=' * 30} Q2 一车多趟最终排班方案 {'=' * 30}\n")
        f_out.write(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f_out.write(f"输入文件: {input_file}\n")
        f_out.write(
            f"运行参数: SPEED=Dynamic(真实时变), SERVICE={SERVICE_TIME}min, TURNAROUND={TURNAROUND_TIME}min\n\n"
        )
        f_out.write(
...
if __name__ == "__main__":
    ...
    report_filename = "Q2_单车单趟明细排班方案.txt"
    with open(report_filename, "w", encoding="utf-8") as f:
        f.write("=== 问题2 单车单趟明细排班方案 ===\n")
    ...
    generate_mtvrp_schedule(report_filename, geo, "Q2一车多趟的最终排班.txt")
